...
if(well_type == "other") {
  if(!file.exists(in_sample)) {
    stm(paste0("ERROR: Cannot find IN Sample Sheet file:", in_sample))
    stop()
  } else {
    stm(paste0("Found Sample Sheet file:", in_sample))
  }
}
```
...
if(well_type == "aifi") {
  ...
  sample_sheet <- read.csv(in_sample)
  batch_id <- sample_sheet$batch_id[sample_sheet$well_id == in_well]
  
  h5_list <- set_list_path(h5_list,
                           "/matrix/observations/batch_id/",
                           rep(batch_id, length(h5_list$matrix$barcodes)))
}
```
