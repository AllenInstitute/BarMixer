...
in_hto_key <- read.csv(in_samples, header = TRUE)
...
rownames(in_hto_mat) <- in_hto_key$hto_barcode[match(rownames(in_hto_mat), in_hto_key$hash_name)]
...
for(sample_id in singlet_summary$sample_id) {
  qc_list$pbmc_sample_hto_stats[[sample_id]]$n_singlets <- singlet_summary$n_singlets[singlet_summary$sample_id == sample_id]
}
