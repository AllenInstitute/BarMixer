...
for(well in names(hto_category_count_by_well)[-1]) {
  ...
  names(pbmc_sample_list) <- hto_barcode_count_by_well$sample_id
...
for(well in names(reads_hto_barcode_by_well)[-c(1,2)]) {
  ...
  names(pbmc_sample_list) <- reads_hto_barcode_by_well$sample_id
...
for(well in names(umis_hto_barcode_by_well)[-c(1,2)]) {
  ...
  names(pbmc_sample_list) <- umis_hto_barcode_by_well$sample_id
...
for(well in names(genes_hto_barcode_by_well)[-c(1,2)]) {
  ...
  names(pbmc_sample_list) <- genes_hto_barcode_by_well$sample_id
